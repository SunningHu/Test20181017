...
//    Write_Value_SOURCE(&data.s16SourceValue, 0, 
//            0, 9999);
...
static  void    DoMEASUREKey(PKeyMsg pKeyMsg)
{
  if(pKeyMsg->u8KeyAct == KEY_ACT_PRESS)
  {
    if((data.u8SSigType >= SIGTYPE_PT100) &&(data.u8SourceEnable == ON))
    {       
//        data.u8SourceEnable = OFF;
//        DrawSourceEnable();
...
static  void    DoTEMPKey(PKeyMsg pKeyMsg)
{
  //if(data.u8MSigType != SIGTYPE_24VLOOP)
  //{
    if((data.u8MSigType > SIGTYPE_100MV && data.u8MSigType < SIGTYPE_PT100) || (data.u8SSigType > SIGTYPE_100MV && data.u8MSigType < SIGTYPE_PT100))  //热电偶
    {
      if(pKeyMsg->u8KeyAct == KEY_ACT_PRESS)
      {
        OnExit();
