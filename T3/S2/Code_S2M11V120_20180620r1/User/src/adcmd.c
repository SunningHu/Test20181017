...
//  ReferValue_b = (calPwm.fSCaLPwm_R[1] * 100 - calPwm.fSCaLPwm_R[0] * 300) / (2 * (calPwm.fSCaLPwm_R[1] - calPwm.fSCaLPwm_R[0]));
}
...
static void	DoCmd11(void)
{
  ...
  {
    data.u8SSigType = SIGTYPE_PT100;
    if(g_ucUsartBuf[2] < 0x0c || g_ucUsartBuf[2] == 0x0e)
      FlagI = 1;  //小档
    else
      FlagI = 0;  //大档
  }
...
static void	DoCmd13(void)
{
  ...
  else if(g_ucUsartBuf[2] == 0x03)//V
    data.u8MSigType = SIGTYPE_V;
  else
  {
    data.u8MSigType = SIGTYPE_PT100;
 //   DoSource(SIGTYPE_MA,100);
  }
  //DrawMeasureData();
  ...
  else if(g_ucUsartBuf[2] == 0x04)
  {
//    g_ucUsartBuf[3] = adCode.u16ResCode1 / 0x100;
//    g_ucUsartBuf[4] = adCode.u16ResCode1 % 0x100;
  }
  else  
  {   
    g_ucUsartBuf[3] = (adCode.u16RCode+CORRECTION_R) / 0x100;
    g_ucUsartBuf[4] = (adCode.u16RCode+CORRECTION_R) % 0x100;
  }
  u16Crc = CalCrc(g_ucUsartBuf, g_ucUsartLen - 2);
  g_ucUsartBuf[5] = u16Crc % 0x100;
...
static	void	DoCmd021101()
{
  ...
  switch(g_ucUsartBuf[4])
  {
    ...
    case 14:data.u8SSigType = 14;data.u8MeasureEnable = 0;break; //欧姆
...
static	void	DoCmd021201()
{
  ...
  switch(g_ucUsartBuf[4])
  {
    ...
    case 14:data.u8MSigType = 14;break; //欧姆
